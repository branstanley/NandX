...
#define PRINT_PREF KERN_INFO "nandx_find_complex: "
...
/* *******************
* Borrowed Call: read_eraseblock_by_page
*   Read NAND one page at a time 
* 
...
/* *******************
* Borrowed Call: dump_eraseblock
*   We are simply using this to print things for now
*		might refactor to search for 0xFF chains and select our target block
* 
* FROM: .../mtd/tests/mtd_readtest.c by Adrian Hunter
* ******************* */ 
...
/* *******************
 * NANDX Call: nandx_print_bad_block
 *  As this finder has more detail, we will gain
 *  introspection by printout out the "bad" block
 *
 * m0nk wrote me!
 * ******************* */ 
static void nandx_print_bad_block(int blockLocation){
  int err = 0;
  
  err = read_eraseblock_by_page(blockLocation);
  
  if(err)
    printk(PRINT_PREF "ERROR on Block load into memory\n");
    //return;
  
  dump_eraseblock(blockLocation);
  
}

/* *******************
 * NANDX Call: nand_find_complex
 ...
 * m0nk wrote me!
 * ******************* */ 
static void nand_find_complex(void)
{
  uint64_t tmp;
  int err, i;
  int bad = 0;
 ...
 *
 * ******************* */ 
static int __init nandx_find_complex_init(void)
{
  //DO: all the things!
  nand_find_complex();
...
module_init(nandx_find_complex_init);

static void __exit nandx_find_complex_exit(void)
{
  return;
}
module_exit(nandx_find_complex_exit);
